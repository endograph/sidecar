...
| per-repo | `.sidecar` config, the checkout (default `sidecar/`), a `.gitignore` entry, an optional `.zed/settings.json` inclusion; the same again per `.sidecar.<peer>`, in `.git/info/exclude` for a peer added with `--ignored` | same |
